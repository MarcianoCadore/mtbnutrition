...
Regras de negócio:
- Um doc de semana tem: {semana_inicio: str (ISO segunda), objetivo: str, treinos: [{...}]}
...
async def _garantir_dia(data_iso: str, db) -> None:
...
async def _set_treino_dia(data_iso: str, campos: dict, db) -> None:
...
async def mover_treino(origem_iso: str, destino_iso: str, modo: str) -> dict:
    ...
    if modo == "swap":
        ...
        await _set_treino_dia(origem_iso, campos_para_origem, db)
        await _set_treino_dia(destino_iso, campos_para_destino, db)
        ...
        await _set_treino_dia(destino_iso, campos_destino, db)
        await _set_treino_dia(origem_iso, campos_origem, db)

        resultado_origem = {"data": origem_iso, **campos_origem}
        resultado_destino = {"data": destino_iso, **campos_destino}

    logger.info("mover_treino: %s → %s (modo=%s)", origem_iso, destino_iso, modo)
    return {
...
async def criar_treino_dia(
    ...
    logger.info("criar_treino_dia: %s tipo=%s dur=%s", data_iso, tipo, duracao_min)
    return {
        "data": data_iso,
        **campos,
